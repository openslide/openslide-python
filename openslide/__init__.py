...
class _OpenSlideCacheWrapper(Protocol):
    _openslide_cache: _OpenSlideCache
...
class _OpenSlideMap(Mapping[str, MapValue]):
    def __init__(self, osr: _OpenSlide):
        self._osr = osr
...
class _PropertyMap(_OpenSlideMap[str]):
    def _keys(self) -> list[str]:
        return lowlevel.get_property_names(self._osr)  # type: ignore
...
class _AssociatedImageMap(_OpenSlideMap[Image.Image]):
    def __init__(self, osr: _OpenSlide, profile: bytes | None):
        _OpenSlideMap.__init__(self, osr)
        self._profile = profile

    def _keys(self) -> list[str]:
        return lowlevel.get_associated_image_names(self._osr)  # type: ignore
...
class OpenSlide(AbstractSlide):
    ...
    def detect_format(cls, filename: Path | str) -> str | None:
        ...
        If the file format is not recognized, return None."""
        return lowlevel.detect_vendor(str(filename))  # type: ignore
    ...
    def level_count(self) -> int:
        """The number of levels in the image."""
        return lowlevel.get_level_count(self._osr)  # type: ignore
    ...
    def get_best_level_for_downsample(self, downsample: float) -> int:
        """Return the best level for displaying the given downsample."""
        return lowlevel.get_best_level_for_downsample(self._osr, downsample)  # type: ignore # noqa: E501
